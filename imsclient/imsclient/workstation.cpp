...
void WorkStation::createUserStateMenu()
{
    ...
    pStateMenu->addAction(tr("系统设置..."));
    pStateMenu->addAction(tr("我的资料..."));
    pStateMenu->addAction(tr("我的QQ中心"));
...
void WorkStation::createSystemTrayIcon()
{
    ...
    trayIconMenu->addAction(tr("打开主界面"), this, SLOT(doActionShowLogin()));
    trayIconMenu->addSeparator();
    trayIconMenu->addAction(tr("退出"), qApp, SLOT(quit()));
...
void WorkStation::addStateAction(QMenu *menu)
{
    ...
    menu->addAction(tr("添加状态信息"));
    menu->addSeparator();
    menu->addAction(tr("关闭所有声音"));
    menu->addAction(tr("关闭头像闪动(仅本次)"));
    menu->addSeparator();
    action = menu->addAction(tr("锁定QQ   Ctrl+Alt+L"), this, SLOT(doActionLock()));
    menu->addSeparator();
}
...
{
    QMenu menu;
    menu.addAction(QIcon(CImsResource::iconInterManager()), tr("打开界面管理器"), this, SLOT(doActionInfoApp()));
...
void WorkStation::doPushButtonContact()
{
    ...
    QMenu *pChildMenu = pMenu->addMenu(QIcon(CImsResource::menuContactHeadIcon()), tr("头像显示"));
    pActionGroup = new QActionGroup(NULL);
    pAction = pChildMenu->addAction(tr("大头像"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pChildMenu->addAction(tr("小头像"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pChildMenu->addSeparator();
    pChildMenu->addAction(tr("选中时显示大头像"))->setEnabled(false);
    pChildMenu->addSeparator();
    pChildMenu->addAction(tr("显示我的头像"))->setCheckable(true);

    pChildMenu = pMenu->addMenu(tr("名称显示"));
    pActionGroup = new QActionGroup(NULL);
    pAction = pChildMenu->addAction(tr("显示备注和昵称"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pAction = pChildMenu->addAction(tr("显示备注"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction = pChildMenu->addAction(tr("显示昵称"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);

    pChildMenu = pMenu->addMenu(tr("列表显示"));
    pActionGroup = new QActionGroup(NULL);
    pAction = pChildMenu->addAction(tr("单列显示"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pAction = pChildMenu->addAction(tr("多列平铺"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pChildMenu->addSeparator();
    pChildMenu->addAction(tr("显示清爽资料"))->setEnabled(true);
    pChildMenu->addSeparator();
    pChildMenu->addAction(tr("动画效果"))->setCheckable(true);

    pMenu->addSeparator();
    pChildMenu = pMenu->addMenu(tr("排序显示"));
    pActionGroup = new QActionGroup(NULL);
    pAction = pChildMenu->addAction(tr("按身份标识排序"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pAction = pChildMenu->addAction(tr("按服务更新排序"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pChildMenu->addAction(tr("按名称排序"));
    pAction = pActionGroup->addAction(pAction);
    pAction->setCheckable(true);

    pMenu->addAction(tr("刷新好友列表"));
    pMenu->addSeparator();
    pMenu->addAction(tr("显示在线联系人"));
    pMenu->addSeparator();
    pMenu->addAction(tr("显示企业分组"));
    pMenu->addAction(tr("显示陌生人分组"))->setCheckable(true);
    pMenu->addAction(tr("显示黑名单"))->setCheckable(true);
}
...
void WorkStation::doPushButtonGroup()
{
    ...
    QAction *pAction = NULL;
    QActionGroup *pActionGroup = NULL;
    QMenu *pChildMenu = pMenu->addMenu(QIcon(CImsResource::menuContactHeadIcon()), tr("图标"));
    pActionGroup = new QActionGroup(NULL);
    pAction = pChildMenu->addAction(tr("大图标"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pAction = pChildMenu->addAction(tr("小图标"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);
    pChildMenu->addSeparator();
    pChildMenu->addAction(tr("选中时显示大图标"))->setEnabled(true);
    ...
    pAction->setCheckable(true);
    pAction->setChecked(true);
    pChildMenu->addAction(tr("多列平铺"));
    pActionGroup->addAction(pAction);
    pAction->setCheckable(true);

    pMenu->addSeparator();
    pMenu->addAction(QIcon(CImsResource::menuGroupIcon()), tr("访问我的群主页"));
    pChildMenu = pMenu->addMenu(tr("所有群消息设置"));
    pChildMenu->addAction(tr("接收并提示信息"));
    pChildMenu->addAction(tr("不提示消息只显示数目"));
    pChildMenu = pChildMenu->addMenu(tr("屏蔽群消息提示"));
    pChildMenu->addAction(tr("15分钟提示一次"));
    pChildMenu->addAction(tr("30分钟提示一次"));
    pChildMenu->addAction(tr("1小时提示一次"));
    pChildMenu->addAction(tr("4小时提示一次"));
    pChildMenu->addSeparator();
    pChildMenu->addAction("始终不提示");

    pMenu->addSeparator();
    pMenu->addAction(tr("查找添加群"));
    pMenu->addAction(tr("创建一个群"));
    pMenu->addSeparator();
    pMenu->addAction(tr("添加群分组"));
}
