...
void TitleWidget::initUi()
{
    ...
    QLabel *titleLabel = new QLabel(QStringLiteral("网易云音乐 1.0.0"));
...
void TitleWidget::initAction()
{
    ...
    loopPlayAction = new QAction(QStringLiteral("全部循环"), m_menu);
    ...
    randomPlayAction = new QAction(QStringLiteral("随机循环"), m_menu);
    ...
    listPlayAction = new QAction(QStringLiteral("列表循环"), m_menu);
    ...
    searchAction = new QAction(QStringLiteral("搜索"), m_menu);
    searchAction->setShortcut(tr("Ctrl+S"));
    searchAction->setStatusTip(QStringLiteral("搜索单曲,歌单..."));
    connect(searchAction, SIGNAL(triggered()), this, SIGNAL(search()));

    logOutAction = new QAction(QStringLiteral("注销"), m_menu);
    logOutAction->setShortcut(tr("Ctrl+E"));
    logOutAction->setStatusTip(QStringLiteral("退出登录..."));
    connect(logOutAction, SIGNAL(triggered()), this, SIGNAL(logOut()));

    switchWidgetAction = new QAction(QStringLiteral("切换到云资源"), m_menu);
    switchWidgetAction->setShortcut(tr("Ctrl+W"));
    connect(switchWidgetAction, SIGNAL(triggered()), this, SIGNAL(switchWidget()));

    QAction *exitAction = new QAction(QStringLiteral("退出软件"), m_menu);
...
void TitleWidget::netWorkModel(bool isTrue)
{
    if (isTrue)
    {
        ...
        switchWidgetAction->setText(QStringLiteral("切换到本地资源"));
    }
    else
    {
        ...
        switchWidgetAction->setText(QStringLiteral("切换到云资源"));
    }
}
