...
void NetWorkWidget::dealLogIn(QNetworkReply *reply)
{
    ...
    case 200:
        break;
    case 408:
        QMessageBox::critical(this, QStringLiteral("登录出错"), QStringLiteral("网络连接超时"), QMessageBox::Ok | QMessageBox::Cancel);
        break;
    case 501:
        QMessageBox::critical(this, QStringLiteral("登录出错"), QStringLiteral("用户名错误"), QMessageBox::Ok | QMessageBox::Cancel);
        break;
    case 502:
        QMessageBox::critical(this, QStringLiteral("登录出错"), QStringLiteral("密码错误"), QMessageBox::Ok | QMessageBox::Cancel);
        break;
    default:
        QMessageBox::critical(this, QStringLiteral("登录出错"), QStringLiteral("未知错误"), QMessageBox::Ok | QMessageBox::Cancel);
        break;
    }
...
void NetWorkWidget::play()
{
    if (m_currentPlayMusic <= 0)
    {
        ...
        if (path == "")
        {
            QMessageBox::critical(this, QStringLiteral("播放出错"),
                                  QStringLiteral("当前播放列表没有资源..."), QMessageBox::Ok | QMessageBox::Cancel);
            m_currentPlayMusic = -1;
            return;
        }
