...
int AsemanDesktopTools::desktopSession() const
{
    ...
    else if(desktop_session->contains("ubuntu",Qt::CaseInsensitive))
    {
        result = AsemanDesktopTools::Unity;
    }
    else if(desktop_session->contains("gnome-fallback",Qt::CaseInsensitive))
    {
...
QColor AsemanDesktopTools::titleBarColor() const
{
    ...
    case AsemanDesktopTools::Gnome:
    {
        ...
        if(!res)
        {
            ...
            else if( sres == "radiance" )
            {
                res = new QColor("#DFD7CF");
            }
            else if( sres == "adwaita" )
            {
                res = new QColor("#EDEDED");
            }
            else if( dsession == AsemanDesktopTools::Unity )
            {
                res = new QColor("#403F3A");
            }
            else
