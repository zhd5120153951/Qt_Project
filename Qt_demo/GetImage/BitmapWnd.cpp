...
CBitmapWnd::CBitmapWnd(IStream *pStream)
    : m_Image(pStream)
{
    m_cRef = 0;
}

CBitmapWnd::CBitmapWnd(PCWSTR pszFileName)
    : m_Image(pszFileName)
{
    m_cRef = 0;
}
...
LRESULT CALLBACK CBitmapWnd::WindowProc(HWND   hWnd, UINT   uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_CREATE:
