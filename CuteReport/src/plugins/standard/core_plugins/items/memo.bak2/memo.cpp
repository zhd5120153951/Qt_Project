...
void MemoItem::adjust(MemoItemPrivate * d, const QPointF &posDeltaMM)
{
    ...
    qreal marginPix = convertUnit(d->textMargin, d->unit, Pixel, d->dpi);
    ...
    if (d->stretchFont && d->stretchMode == DontStretch) {
        ...
        d->adjustedFontPointSize = convertUnit( fontFrame, d->unit, Pixel, d->dpi).height() * 0.85;
    } else {
    ...
    if (d->stretchMode != DontStretch || d->allowHtml) {
        ...
        d->textDocument->setTextWidth(convertUnit(qrect, Millimeter, Pixel, d->dpi).width() - 2*marginPix);
        ...
        if ((d->textDocument && !d->textDocument->toPlainText().isEmpty()) &&
                ((d->stretchMode == ActualHeight && d->renderingType == RenderingReport)|| (d->stretchMode == ActualHeight && d->renderingType == RenderingTemplate && d->showStretchability)))  {
            //        if (d->stretchMode == ActualHeight) {
            qreal textHeight = convertUnit(d->textDocument->size().height(), Pixel, Millimeter, d->dpi) + d->textMargin*2;
            if (textHeight > d->rect.height()) {
                if (d->originalHeight == -1 && d->renderingType == RenderingTemplate) // not stretched state
...
{
    qDebug() << stretchModeStr() << stretchMode();
    emit printBefore();
...
CuteReport::RenderedItemInterface * MemoItem::renderView()
{
    Q_D(MemoItem);
    RenderedMemoItem * view = new RenderedMemoItem(this, new MemoItemPrivate(*d));
    return view;
}
...
void MemoItem::paint(QPainter * painter, const QStyleOptionGraphicsItem *option, const BaseItemInterfacePrivate * data, const QRectF &boundingRect, RenderingType type)
{
    ...
    qreal marginPix = convertUnit(d->textMargin, Millimeter, Pixel, d->dpi);
    ...
    if (!d->textDocument) {
        if (d->adjustedFontPointSize > 3) {
            ...
            painter->drawText(marginPix, marginPix, rect.width() - marginPix*2, rect.height() - marginPix*2, d->textFlags, d->text);
        }
    } else {
        qreal x = marginPix;
        qreal y = marginPix;
        ...
        if (d->textFlags.testFlag(AlignHCenter))
            x = ((qreal)rect.width()- docSize.width() )/2;
        if (d->textFlags.testFlag(AlignVCenter))
            y = ((qreal)rect.height() - docSize.height() )/2;
            //y = (((qreal)rect.height() -2*marginPix) - docSize.height() )/2;
        painter->save();
